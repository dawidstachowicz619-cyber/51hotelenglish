"use client";

import { useCallback, useEffect, useState } from "react";
import { Briefcase, CheckCircle2, Play, Users } from "lucide-react";

import { HrTrainingLesson } from "@/components/grow-in-hotel/hr-training-lesson";
import { Button } from "@/components/ui/button";
import { totalVideoDurationSec } from "@/lib/hr/document-processor";
import {
  getVisibleManagementModules,
  isManagerRole,
} from "@/lib/hr/management-training-storage";
import {
  getModuleScore,
  isModuleCompleted,
} from "@/lib/hr/training-progress-storage";
import type { EmployeeDepartment } from "@/lib/types/hr-admin";
import { ASK_SHORT } from "@/lib/types/learning-record";
import type { HrTrainingModule } from "@/lib/types/hr-training";
import { cn } from "@/lib/utils";

type ManagementTrainingSectionProps = {
  hotel: string;
  department: EmployeeDepartment;
  employeeRole?: string;
};

export function ManagementTrainingSection({
  hotel,
  department,
  employeeRole = "",
}: ManagementTrainingSectionProps) {
  const [modules, setModules] = useState<HrTrainingModule[]>([]);
  const [active, setActive] = useState<HrTrainingModule | null>(null);
  const [, tick] = useState(0);

  const refresh = useCallback(() => {
    setModules(getVisibleManagementModules(hotel, department));
    tick((n) => n + 1);
  }, [hotel, department]);

  useEffect(() => {
    refresh();
    window.addEventListener("hr-training-updated", refresh);
    window.addEventListener("employee-training-updated", refresh);
    return () => {
      window.removeEventListener("hr-training-updated", refresh);
      window.removeEventListener("employee-training-updated", refresh);
    };
  }, [refresh]);

  if (active) {
    return (
      <HrTrainingLesson
        module={active}
        onBack={() => setActive(null)}
        onComplete={() => {
          setActive(null);
          refresh();
        }}
      />
    );
  }

  const completedCount = modules.filter((m) => isModuleCompleted(m.id)).length;
  const forManagers = isManagerRole(employeeRole);

  return (
    <section id="management-training" className="scroll-mt-24">
      <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <div className="flex items-center gap-2">
            <Briefcase className="size-5 text-accent" strokeWidth={2.25} />
            <h2 className="font-display text-xl text-foreground">Management Training</h2>
          </div>
          <p className="mt-1 text-sm font-semibold text-muted-foreground">
            管理培训 · 面向主管、经理与储备干部
            {modules.length > 0 && (
              <span className="ml-1 font-extrabold text-foreground">
                · {completedCount}/{modules.length} 已完成
              </span>
            )}
          </p>
        </div>
        {forManagers && (
          <span className="inline-flex w-fit items-center gap-1 rounded-full bg-accent/15 px-3 py-1 text-[10px] font-extrabold text-accent">
            <Users className="size-3" />
            管理岗位推荐
          </span>
        )}
      </div>

      <div className="grid gap-4 sm:grid-cols-2">
        {modules.map((mod) => {
          const done = isModuleCompleted(mod.id);
          const score = getModuleScore(mod.id);
          const mins = Math.max(1, Math.ceil(totalVideoDurationSec(mod) / 60));

          return (
            <div
              key={mod.id}
              className={cn(
                "card-elevated flex flex-col border-2 p-5 transition-all hover:-translate-y-0.5",
                done ? "border-accent/35 bg-accent/5" : "border-border"
              )}
            >
              <div className="flex items-start justify-between gap-2">
                <span className="flex size-10 items-center justify-center rounded-xl bg-accent/15 text-accent">
                  <Briefcase className="size-5" />
                </span>
                {done && (
                  <span className="inline-flex items-center gap-1 rounded-full bg-primary-light px-2 py-0.5 text-[10px] font-extrabold text-primary">
                    <CheckCircle2 className="size-3" />
                    已完成 {score}%
                  </span>
                )}
              </div>

              <h3 className="mt-3 font-display text-lg text-foreground">{mod.title}</h3>
              <p className="mt-1 flex-1 text-xs font-semibold text-muted-foreground">
                {mod.deliveryType === "video" ? (
                  <>视频课 · {mod.questionCount} 道测验</>
                ) : (
                  <>
                    {mod.slideCount} 节讲解 · {mod.questionCount} 道测验 · ~{mins} 分钟
                  </>
                )}
              </p>
              <p className="mt-1 text-[10px] font-bold text-muted-foreground">
                Management Training · {ASK_SHORT[mod.ask]}
              </p>

              <Button
                className="mt-4 w-full"
                variant={done ? "outline" : "default"}
                onClick={() => setActive(mod)}
              >
                <Play className="size-4" />
                {done ? "重新学习" : "开始学习"}
              </Button>
            </div>
          );
        })}
      </div>
    </section>
  );
}
