...
import {
  buildRoomItemPracticeQuestions,
  pickRandomRoomItemSubset,
} from "@/lib/course/room-item-practice";
...
export function RoomItemsPracticeTab() {
  const [mode, setMode] = useState<PracticeMode>("image");
  const [seed, setSeed] = useState(1);

  const subset = useMemo(
    () => pickRandomRoomItemSubset(HOTEL_RUSSIAN_ROOM_ITEMS, BATCH_SIZE, seed),
    [seed]
  );

  const questions = useMemo(
    () => buildRoomItemPracticeQuestions(subset, mode),
    [subset, mode]
  );
  ...
  const currentItemId = question?.id.match(/^ri-q-(ri-\d+)-/)?.[1];
  const currentItem = currentItemId ? getRoomItemById(currentItemId) : undefined;
  ...
  const finishSession = () => {
    ...
    const result = logRussianItemsPracticeSession("room", {
      score,
      correctCount,
        ...
        <div className="p-6 md:p-8">
          {currentItem && mode === "image" && (
            <div className="mx-auto mb-6 max-w-md overflow-hidden rounded-2xl border-2 border-border bg-white">
              <RoomItemImage item={currentItem} className="aspect-square max-h-56" />
              <div className="border-t-2 border-border bg-[#0039A6]/5 px-4 py-4 text-center">
                <span className="inline-block rounded-full bg-white px-2.5 py-0.5 text-[10px] font-extrabold text-[#0039A6]">
                  {ROOM_ITEM_CATEGORY_LABELS[currentItem.category].zh}
                </span>
          ...
          {currentItem && mode === "text" && (
            <div className="mb-6 rounded-2xl border-2 border-[#0039A6]/15 bg-[#0039A6]/5 px-5 py-4 text-center">
              <span className="inline-block rounded-full bg-white px-2.5 py-0.5 text-[10px] font-extrabold text-[#0039A6]">
                {ROOM_ITEM_CATEGORY_LABELS[currentItem.category].zh}
              </span>
